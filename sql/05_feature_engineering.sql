...
base_metrics AS (
    SELECT
        ...
        (ip.annual_demand - ip.inventory_units_on_hand) AS obsolete_inventory,
        -- Normalize demand variability to a 0–1 scale
        ((ip.demand_variability_cov - c.minimum_cov) / NULLIF(c.maximum_cov - c.minimum_cov, 0)) AS normalized_cov,
        -- Average daily demand based on a 360-day planning year
        (ip.annual_demand / c.planning_days) AS daily_demand,
...
lead_time_metrics AS (
    SELECT
        bm.*,
        (bm.daily_demand * bm.lead_time_days) AS demand_during_lead_time
...
variability_metrics AS (
    SELECT
        ltm.*,
        (ltm.normalized_cov * ltm.demand_during_lead_time) AS standard_deviation_during_lead_time
...
safety_stock_metrics AS (
    SELECT
        vm.*,
        (vm.service_level_z_score * vm.standard_deviation_during_lead_time) AS safety_stock
...
SELECT
    ...
    ssm.safety_stock,
    (ssm.demand_during_lead_time + ssm.safety_stock) AS reorder_point
